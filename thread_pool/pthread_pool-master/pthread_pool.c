...
tp_thread_pool *creat_thread_pool(int min_num, int max_num){
	...
	//初始化线程池大小
	this->min_th_num = min_num; //最小线程数
	this->cur_th_num = this->min_th_num; //当前线程数为最小线程数
	this->max_th_num = max_num; //最大线程数
	this->idle_th_num = this->cur_th_num; //空闲线程数为当前线程数
 
	//初始化线程池互斥锁
	pthread_mutex_init(&this->tp_lock, NULL); 

	//根据线程的最大值，为线程结构分配空间		
	if(NULL != this->thread_info) //如果线程头不空
		free(this->thread_info);
	this->thread_info = malloc_pthread(this->cur_th_num);

	if(NULL == this->thread_info){ //如果创建的是空, 则释放对象, 返回错误
		free(this);
		return NULL;
	}
	//初始化线程池
...
int tp_init(tp_thread_pool *this){
	...
	//创建线程并初始化线程结构/创建所有线程, 已经分配了内存
	for(thread_info = this->thread_info;thread_info!=NULL;thread_info = thread_info->next){
		if(creat_thread(thread_info,this)) //将这个线程给线程池this <--- thread_info
			printf("tp_init: creat work thread %lu\n", thread_info->thread_id); //每个线程创建成功打印id
		else{
